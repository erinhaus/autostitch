...
def computeHomography(f1, f2, matches, A_out=None):
    ...
    for i in range(len(matches)):
        ...
        # For each match, fill in the two corresponding rows of the A matrix
        # from the equation Ah=0
        A[2*i] = [a_x,a_y,1,0,0,0,(-1*b_x*a_x),(-1*b_x*a_y),(-1*b_x)]
        A[2*i+1] = [0,0,0,a_x,a_y,1,(-1*b_y*a_x),(-1*b_y*a_y),(-1*b_y)]
    ...
    # Since H is a 3x3 right singular vector that corresponds to the smallest
    # singular value of A (size in S, which is ordered largest to smallest),
    # then we choose the eigenvector of AtA associated with this scale, which
    # are the values are the last column of V, or the last row of Vt.

    # Grab last row (length=9) of Vt which contains the values of H
    H = Vt[-1].reshape(3,3)
...
# Initialize maximum inlier counter
    max_inliers = 0

    for i in range(nRANSAC):
        # Choose s samples (>= samples to fit model - 3x3 so 3)
        if m == eHomography:
            samples = np.random.randint(0,len(matches),4)
        elif m == eTranslate:
            samples = np.random.randint(0,len(matches),1)

        # Get list of those matches
        sample_matches = [matches[s] for s in samples]

        # Fit model
        homography = computeHomography(f1,f2,sample_matches)

        # Find inliers for model
        inliers = getInliers(f1, f2, matches, homography, RANSACthresh)

        # Keep track of best-performing model
        if len(inliers) > max_inliers:

            max_inlier_indices = inliers
...
def getInliers(f1, f2, matches, M, RANSACthresh):
    ...
    for i in range(len(matches)):
        ...
        # Grab match
        m = matches[i]
        # Get coordinates associated with feature in each image
        (a_x, a_y) = f1[m.queryIdx].pt
        (b_x, b_y) = f2[m.trainIdx].pt

        # Make feature in first image homogenous for matrix multiplication
        homogenous_a = [a_x,a_y,1]

        # Perform transformation on feature
        transformed = M @ homogenous_a

        # Grab normalized x', y'
        trans_x = transformed[0]/transformed[2]
        trans_y = transformed[1]/transformed[2]

        # Find Euclidean distance between (x,y) and (x',y')
        dist = math.sqrt((b_x-trans_x)**2+(b_y-trans_y)**2)

        # Add to inlier list if within RANSAC threshold
...
def leastSquaresFit(f1, f2, matches, m, inlier_indices):
    ...
    if m == eTranslate:
        ...
        for i in range(len(inlier_indices)):
            ...
            # Add distance between each df of features between images
            # (perform division to complete average outside of TODO block)
            u += (b_x-a_x)
            v += (b_y-a_y)
        ...
        # Get matches that are inliers according to model + RANSAC threshold
        matches = [matches[i] for i in inlier_indices]

        # Compute the best homography for inliers
